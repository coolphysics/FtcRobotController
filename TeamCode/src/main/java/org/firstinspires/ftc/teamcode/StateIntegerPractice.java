...
public class StateIntegerPractice extends OpMode {
    ...
    // int state; instead of using numbers for state, more descriptive using enum
    enum State {
        WAIT_FOR_A,
        WAIT_FOR_B,
        WAIT_FOR_X,
    ...
    public void init() {
        ...
        // state = 0; // leave for now, later change to text-based, so below change state numbers
    ...
    public void loop() {
        ...
        switch (state) {
            ...
            case WAIT_FOR_X:
                ...
                break;
            default:
                telemetry.addLine("Auto state machine is finished");
        }
